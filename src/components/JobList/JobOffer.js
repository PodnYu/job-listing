...
export default function JobOffer({ offer }) {
  ...
  const { isOfferValid, addTag } = useContext(FilterContext);

  const tag = (content, index) => <div key={index} className="tag" onClick={() => addTag(content)}>{content}</div>;
  ...
  return (
    <>
      {isOfferValid(offer) && <div className="offer" style={styles}>
